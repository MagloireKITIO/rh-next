...
export class ProjectsAnalyticsService {
  private readonly logger = new Logger(ProjectsAnalyticsService.name);
  ...
  async getProjectsAnalytics(
    ...
    let query = this.dataSource
      ...
      .groupBy('project.id, project.name, project.status, project.createdAt, project.updatedAt');
  ...
  async exportProjectReport(
    projectId: string,
    companyId: string,
    options: { format: 'pdf' | 'excel'; period?: string }
  ): Promise<Buffer> {
    const report = await this.getProjectReport(projectId, companyId, { period: options.period });
  ...
  private async generateExcelReport(report: ProjectReport): Promise<Buffer> {
    ...
    workbook.creator = 'ATS Professional';
  ...
  async getProjectBasicInfo(projectId: string, companyId: string) {
    const project = await this.projectRepository.findOne({
      where: { id: projectId, company_id: companyId },
      select: ['id', 'name'],
    });
